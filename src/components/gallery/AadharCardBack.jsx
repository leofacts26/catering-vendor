...
const AadharCardBack = () => {
    ...
    const onHandleSubmit = async (event) => {
        ...
        try {
            if (photoURL) {
                if (settings['vendor-enca-back']?.length && settings['vendor-enca-back']?.length > 0) {
                    await onReUploadBackAadharLogo();
                } else {
                    await onUploadBackAadharLogo();
                }
            } else {
    ...
    return (
        <>
                ...
                <div className="mt-3 text-center">
                    <Stack direction="row" justifyContent="center">
                        ...
                        <Button onClick={handleBrandClickOpen} variant="contained" component="span" className="cuisines-list-white-btn"
                            disabled={isLoading || !(settings['vendor-enca-back']?.length && settings['vendor-enca-back']?.length > 0)}
                        >
                            Delete
                        </Button>
                    </Stack>
            ...
            <DeleteModal
                ...
                onHandleRemoveModalLogo={onHandleRemoveBackAadharLogo} />
                            ...
                            <div className="text-center">
                                {settings['vendor-enca-back']?.length && settings['vendor-enca-back']?.length > 0 ? (
                                    <>
                                        <input
                                            accept="image/*"
                                            id="onReUploadBackAadharBanner"
                                            multiple
                                            type="file"
                                            style={{ display: 'none' }}
                                            onChange={onReUploadBackAadharBanner}
                                        />
                                        <label htmlFor="onReUploadBackAadharBanner">
                                    ...
                                    <>
                                        <input
                                            accept="image/*"
                                            id="onUploadBackAadharBanner"
                                            multiple
                                            type="file"
                                            style={{ display: 'none' }}
                                            onChange={onUploadBackAadharBanner}
                                        />
                                        <label htmlFor="onUploadBackAadharBanner">
                                            <Button variant="contained" component="span" className="upload-btn" disabled={isLoading}>
                            ...
                            <div>
                                ...
                                <label htmlFor="mainbannerlogo">
                                    <Button variant="contained" component="span" className="cuisines-list-white-btn" disabled={isLoading}>
                                        {settings['vendor-enca-back']?.length && settings['vendor-enca-back']?.length > 0 ? 'Re Upload Crop Image' : 'Upload Crop Image'}
                                    </Button>
                                </label>
                            </div>
                        </Stack>
