...
const GstinPhoto = () => {
    ...
    // console.log(settings['vendor-encg']?.length && settings['vendor-encg']?.length > 0, "settings");
    ...
    const onHandleSubmit = async (event) => {
        ...
        try {
            if (photoURL) {
                if (settings['vendor-encg']?.length && settings['vendor-encg']?.length > 0) {
                    await onReUploadGstinLogo();
                } else {
                    await onUploadGstinLogo();
                }
            } else {
    ...
    return (
        <>
            <div>
                <Accordion className="faq-bg">
                    <AccordionSummary
                        ...
                        id="panel1-header"
                    >
                        <p className="settings-faq-title" style={{ fontSize: '14px', fontWeight: '500' }}> GSTIN Card Upload </p>
                    </AccordionSummary>
                    <AccordionDetails>
                        {
                            settings['vendor-encg'] !== undefined ? (
                                <>
                                    {settings['vendor-encg']?.map((logo, index) => (
                                        <img
                        ...
                        <p className="settings-small mt-1">Upload GSTIN Card</p>
                        ...
                        <Stack direction="row" justifyContent="center">
                            ...
                            <Button onClick={handleBrandClickOpen} variant="contained" component="span" className="cuisines-list-white-btn"
                                disabled={isLoading || !(settings['vendor-encg']?.length && settings['vendor-encg']?.length > 0)}
                            >
                                Delete
                            </Button>
                        </Stack>
            ...
            <DeleteModal
                ...
                onHandleRemoveModalLogo={onHandleRemoveGstinLogo} />
                    ...
                    <DialogContent dividers>
                        <Stack direction="row" alignItems="center" justifyContent="center" spacing={2} >
                            ...
                            <div className="text-center">
                                {settings['vendor-encg']?.length && settings['vendor-encg']?.length > 0 ? (
                                    <>
                                        <input
                                            accept="image/*"
                                            id="onReUploadGstinBanner"
                                            multiple
                                            type="file"
                                            style={{ display: 'none' }}
                                            onChange={onReUploadGstinBanner}
                                        />
                                        <label htmlFor="onReUploadGstinBanner">
                                    ...
                                    <>
                                        <input
                                            accept="image/*"
                                            id="onUploadGstinBanner"
                                            multiple
                                            type="file"
                                            style={{ display: 'none' }}
                                            onChange={onUploadGstinBanner}
                                        />
                                        <label htmlFor="onUploadGstinBanner">
                                            <Button variant="contained" component="span" className="upload-btn" disabled={isLoading}>
                            ...
                            <div>
                                ...
                                <label htmlFor="mainbannerlogo">
                                    <Button variant="contained" component="span" className="cuisines-list-white-btn" disabled={isLoading}>
                                        {settings['vendor-encg']?.length && settings['vendor-encg']?.length > 0 ? 'Re Upload Crop Image' : 'Upload Crop Image'}
                                    </Button>
                                </label>
                            </div>
                        </Stack>
